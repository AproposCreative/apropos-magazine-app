...
class SecureConfig {
    ...
    /// The OpenAI API key intentionally no longer ships in the app.
    ///
    /// Recommendation reasons are generated by the `generateRecommendationReasons`
    /// Cloud Function which holds the key as a Firebase Secret. Only an env var is
    /// honoured for local development; release builds resolve to an empty string.
    var openAIAPIKey: String {
        ProcessInfo.processInfo.environment["OPENAI_API_KEY"] ?? ""
    }

    /// Endpoint for the Cloud Function that returns AI recommendation reasons.
    var recommendationReasonsURL: URL? {
        if let rawValue = secretValue(
            for: "RECOMMENDATION_REASONS_URL",
            service: "recommendation_reasons_url",
            envKey: "RECOMMENDATION_REASONS_URL"
        ), let url = URL(string: rawValue) {
            return url
        }
        return URL(string: "https://us-central1-apropos-magazine-6004a.cloudfunctions.net/generateRecommendationReasons")
    }
