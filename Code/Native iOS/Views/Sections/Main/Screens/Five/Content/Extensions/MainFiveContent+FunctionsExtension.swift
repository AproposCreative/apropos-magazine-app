//
//  MainFiveContent+FunctionsExtension.swift
//  Native
//

import Foundation

extension MainFiveContentView {
    
    // MARK: - Functions:
    
    /// Returns the content of the given comment:
    func content(_ comment: NT_Comment) -> String {
        comment.content
    }
    
    /// Returns the title of the given project:
    func title(_ project: NT_Project) -> String {
        project.title
    }
    
    /// Fetches the data to display:
    func fetchData() async {
        
        /// Firstly making the async task sleep for one second that is transformed into nanoseconds to simulate a network call (Please remove this code in production):
        try? await Task.sleep(nanoseconds: UInt64(1 * 1_000_000_000))
        
        /// Then updating the UI on the main thread:
        await MainActor.run {
            
            /// Then getting an array of the tasks that are based on an array of the example tasks:
            let tasks: [NT_Task] = NT_Task.example
            
            /// Then assigning an array of the tasks to display:
            self.tasks = tasks
            
            /// Then also getting an array of the comments that are based on an array of the example comments:
            let comments: [NT_Comment] = NT_Comment.example
            
            /// Then assigning an array of the comments to display:
            self.comments = comments
            
            /// Then also getting an array of the projects that are based on an array of the example projects:
            let projects: [NT_Project] = NT_Project.example
            
            /// Then assigning an array of the projects to display:
            self.projects = projects
        }
        
        /*
         
         NOTE: You can add your own logic for fetching the data here.
         
         */
        
        /// Lastly setting the 'isFetching' property to 'false' to stop fetching the data:
        updateIsFetching(false)
    }
    
    /// Shows the settings screen:
    func showSettings() {
        
        /*
         
         NOTE: You can add your own logic for showing the settings here.
         
         */
        
    }
    
    // MARK: - Private functions:
    
    /// Updates the 'isFetching' property with the given value:
    @MainActor
    private func updateIsFetching(_ isFetching: Bool) {
        
        /// Firstly making sure that the 'isFetching' property isn't the same as the given value:
        if self.isFetching != isFetching {
            
            /// If yes, then updating the 'isFetching' property with the given value:
            self.isFetching = isFetching
        }
    }
}
