//
//  MainTwoContent+FunctionsExtension.swift
//  Native
//

import Foundation

extension MainTwoContentView {
    
    // MARK: - Functions:
    
    /// Returns the title of the given category:
    func title(_ category: NT_CoursesCategory) -> String {
        category.title
    }
    
    /// Fetches the data to display:
    func fetchData() async {
        
        /// Firstly making the async task sleep for one second that is transformed into nanoseconds to simulate a network call (Please remove this code in production):
        try? await Task.sleep(nanoseconds: UInt64(1 * 1_000_000_000))
        
        /// Then updating the UI on the main thread:
        await MainActor.run {
            
            /// Then getting an array of the 'Just for You' courses that are based on an array of the example courses:
            let justForYouCourses: [NT_Course] = .init(NT_Course.example.prefix(3))
            
            /// Then assigning an array of the 'Just for You' courses to display:
            self.justForYouCourses = justForYouCourses
            
            /// Then also getting an array of the popular categories that are based on an array of the example popular categories:
            let popularCategories: [NT_CoursesCategory] = NT_CoursesCategory.example
            
            /// Then assigning an array of the popular categories to display:
            self.popularCategories = popularCategories
            
            /// Then getting an array of the user's courses that are based on an array of the example courses:
            let myCourses: [NT_Course] = .init(NT_Course.example.suffix(5))
            
            /// Then assigning an array of the user's courses to display:
            self.myCourses = myCourses
        }
        
        /*
         
         NOTE: You can add your own logic for fetching the data here.
         
         */
        
        /// Lastly setting the 'isFetching' property to 'false' to stop fetching the data:
        updateIsFetching(false)
    }
    
    // MARK: - Private functions:
    
    /// Updates the 'isFetching' property with the given value:
    @MainActor
    private func updateIsFetching(_ isFetching: Bool) {
        
        /// Firstly making sure that the 'isFetching' property isn't the same as the given value:
        if self.isFetching != isFetching {
            
            /// If yes, then updating the 'isFetching' property with the given value:
            self.isFetching = isFetching
        }
    }
}
